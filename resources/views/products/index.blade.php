...
@section('konten')
        ...
        @if (auth()->user()?->isAdmin())
            <div class="flex items-center gap-2">
                <a href="{{ route('products.arsip') }}"
                   class="rounded border bg-white px-4 py-2 text-sm hover:bg-gray-50">
                    Arsip
                </a>
    ...
    <form action="{{ route('products.index') }}" method="GET" class="mb-4 flex flex-wrap gap-2">
        ...
        @if ($q !== '' || $categoryId)
            <a href="{{ route('products.index') }}" class="rounded border bg-white px-4 py-2 text-sm hover:bg-gray-50">Reset</a>
        @endif
    </form>
                            ...
                            <td class="px-4 py-3">
                                <div class="flex flex-wrap justify-end gap-1.5">
                                    ...
                                    <form action="{{ route('products.nonaktifkan', $product) }}" method="POST"
                                        ...
                                        @csrf
                                        <button class="inline-flex items-center rounded-md border border-orange-200 bg-orange-50 px-2.5 py-1 text-xs font-medium text-orange-700 hover:bg-orange-100">
                                            Nonaktifkan
                                        </button>
                                    </form>
                                </div>
    ...
    @else
        ...
        <div class="grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-4">
            @foreach ($products as $product)
                <a href="{{ route('products.show', $product) }}"
                   class="rounded-lg bg-white p-3 shadow hover:shadow-md">
                    @if ($product->primaryImage)
                        <img src="{{ $product->primaryImage->url }}" alt="{{ $product->name }}"
                             loading="lazy"
